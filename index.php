<!DOCTYPE html>
<html lang="pt-BR">
    <head>
        <title>MA225 - Análise de Livros e Materiais Didáticos de Matemática</title>

        <?php include('assets/php/head.php'); ?>

        <link rel="preload" as="style" onload="this.remove();" href="/~ma225/assets/css/home.css?t=<?php echo date('YmdHis'); ?>" type="text/css">
        <link rel="stylesheet" href="/~ma225/assets/css/home.css?t=<?php echo date('YmdHis'); ?>" type="text/css">

        <!-- SEO -->
        <meta name="author" content="Speck">
        <meta name="description" content="Site Institucional da Disciplina MA225 oferecida pelo IMECC-Unicamp">
        <meta name="keywords" content="ma225, livros didáticos, materiais didáticos, matemática, imecc, unicamp">
        <link rel="canonical" href="https://ime.unicamp.br/~ma225">
    </head>
    <body>
        <?php include('assets/php/header.php'); ?>
        <main>
            <section>
                <h1>Sobre a Disciplina</h1>
                <p>A disciplina MA225 — Análise de Livros Didáticos de Matemática tem como objetivo promover uma reflexão crítica sobre o papel do livro didático no ensino de Matemática, especialmente no contexto da educação básica brasileira. Ao longo do semestre, estudantes são convidados a investigar diferentes materiais didáticos à luz de referenciais curriculares e institucionais, como a BNCC, o PNLD e outros documentos orientadores, desenvolvendo critérios próprios de análise e avaliação.</p>
                <p>A proposta da disciplina é essencialmente investigativa e colaborativa. Organizados em grupos, os participantes realizam estudos sistemáticos de livros didáticos por meio de diferentes abordagens analíticas, como análises verticais (avaliação aprofundada de uma obra) e horizontais (comparação entre materiais distintos). Nesse processo, são discutidos aspectos como estrutura editorial, linguagem e abordagem pedagógica, definição de conceitos, qualidade de exemplos e exercícios, e nível de demanda cognitiva das atividades.</p>
                <p>Além do conteúdo teórico, a disciplina valoriza a produção acadêmica e o trabalho em equipe. Os estudantes elaboram relatórios escritos, apresentações orais e metodologias próprias de análise, exercitando habilidades de argumentação, sistematização e comunicação científica. Dessa forma, MA225 contribui tanto para a formação crítica de futuros professores e pesquisadores quanto para a compreensão do livro didático como objeto central nas práticas de ensino e aprendizagem em Matemática.</p>
            </section>
            <section id="useful_links" class="gallery center">
                <a href="https://drive.google.com/drive/folders/1plVSxOqc9XIfBGpBkDN-lNucwVjnNGKv?usp=drive_link" class="button">Apresentação da Disciplina</a>
                <a href="https://drive.google.com/open?id=1dL6IVZPR3vT9D9iCIK9CD1dEjbLyKT2hdzjCoMSLL4A&usp=drive_copy" class="button">Grupos de Trabalho</a>
            </section>
        </main>
        <?php include('assets/php/footer.php'); ?>
        <script src="/~ma225/assets/js/detectScroll.js"></script>
    </body>
</html>